  ...
  <app-content title="Milestone / Todo">
    ...
    <div class="max-w-2xl mx-auto py-2 px-6 bg-white rounded-2xl border-2 border-gray-200">
      <h3 class="pt-4 pb-2 font-bold text-xl font-serif">To Do
        <span class="font-normal text-base">[{{ sortedTodos.filter(todo => getTodoName(todo).charAt(0) !== '#').length +
            '/' +
            sortedTodos.length
        }}]</span>
      </h3>
      <div class="w-full bg-gray-200 rounded-full h-1.5 mb-4">
        <div class="bg-gray-600 h-1.5 rounded-full"
          :style="[{ 'width': (sortedTodos.filter(todo => getTodoName(todo).charAt(0) === '#').length / sortedTodos.length) * 100 + '%' }]">
        </div>
      </div>
      <div class="divide-y-2 divide-gray-200">
        <Disclosure v-for="(todo, index) in sortedTodos" :key="index" v-slot="{ open }">
          <DisclosureButton class="flex items-start justify-between py-2 w-full">
            <span class="mr-4 font-bold">
              {{ (index + 1).toString().length === 1 ? '0' + (index + 1) : index + 1 }}
            </span>
            <div :class="{ 'line-through': getTodoName(todo).charAt(0) === '#' }" class="text-left font-bold w-full"
              v-html="labelTodo(getTodoName(todo))"></div>
            <ChevronUpIcon v-if="todo.desc" class="h-6 w-6"
              :class="open ? 'transform rotate-180 transition duration-200' : ''" />
            <svg xmlns="http://www.w3.org/2000/svg"
              :class="[getTodoName(todo).charAt(0) === '#' ? 'text-white bg-green-400' : 'text-gray-500 border border-gray-500']"
              class="h-5 w-5 ml-4 p-0.5 rounded-full shrink-0" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd"
                d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                clip-rule="evenodd" />
            </svg>
          </DisclosureButton>
          <DisclosurePanel v-if="todo.desc" class="p-2 bg-gray-100 text-gray-700 whitespace-pre-line">
            {{ todo.desc }}
          </DisclosurePanel>
        </Disclosure>
      </div>
...
let todos = ref([
  ...
  "Theme: Cyberpunk",
  {
    name: "#Modular sidebar app content",
...
const sortedTodos = computed(() => {
  return JSON.parse(JSON.stringify(todos.value)).sort((a, b) => {
    if (getTodoName(a).charAt(0) !== '#' && getTodoName(b).charAt(0) === '#')
      return -1;
